...
<div class="p-6 space-y-4">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('barang.keluar.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Tambah</a>
    </div>
                ...
                @forelse ($items as $row)
                    <tr>
                        ...
                        <td class="px-4 py-2 text-right space-x-2">
                            <a href="{{ route('barang.keluar.edit', $row) }}" class="px-3 py-1 rounded bg-yellow-500 text-white hover:bg-yellow-600">Edit</a>
                            <form action="{{ route('barang.keluar.destroy', $row) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data?')">
                                @csrf
                                @method('DELETE')
